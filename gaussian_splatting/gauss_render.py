...
def build_rotation(r, device: Union[torch.device, str] = 'cuda'):
...
def build_scaling_rotation(s, r, device: Union[torch.device, str] = 'cuda'):
...
def build_covariance_3d(s, r, device: Union[torch.device, str] = 'cuda'):
    L = build_scaling_rotation(s, r, device=device)
...
def build_covariance_2d(
    ...
    tan_fovx = np.tan(fov_x * 0.5)
    tan_fovy = np.tan(fov_y * 0.5)
...
def projection_ndc(points, viewmatrix, projmatrix, depth_thresh: float = 0.2):
    points_o = homogeneous(points)  # object space
    points_h = points_o @ viewmatrix @ projmatrix  # screen space # RHS
    p_proj = points_h / (points_h[..., -1:] + 0.000001)
    p_view = points_o @ viewmatrix
    in_mask = p_view[..., 2] >= depth_thresh
...
class GaussRenderer(nn.Module):
    ...
    def forward(self, camera, pc, **kwargs):
        ...
        with prof("projection"):
            mean_proj, mean_view, in_mask = projection_ndc(means3D,
                                                           viewmatrix=camera.world_view_transform,
                                                           projmatrix=camera.projection_matrix)
            mean_proj = mean_proj[in_mask]
            mean_view = mean_view[in_mask]
            depths = mean_view[:, 2]
            mean_coord_x = ((mean_proj[..., 0] + 1) * camera.image_width - 1.0) * 0.5
            mean_coord_y = ((mean_proj[..., 1] + 1) * camera.image_height - 1.0) * 0.5
